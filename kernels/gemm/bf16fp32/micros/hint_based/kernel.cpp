...
void micro_tk(const micro_globals g) {
    ...
    constexpr int cluster_mfma_count_val = cluster_mfma_count<REG_BLOCK_M, REG_BLOCK_N, DOT_SLICE>();
    constexpr int vmem_loads_val = compute_buffer_loads<BLOCK_SIZE, K_STEP, sizeof(T), NUM_THREADS>();
    constexpr int ds_reads_a_val = compute_ds_reads<A_tile.height, A_tile.width>();
    constexpr int ds_reads_b_val = compute_ds_reads<B_tile.height, B_tile.width>();
    ...
    for (int tile = 0; tile < num_tiles - 1; ++tile, tic^=1, toc^=1) {
        ...
        for (int i = 0; i < vmem_loads_val + vmem_loads_val; i++) {
            schedule_group_barrier(VMEM_READ, 1, 0);
            schedule_group_barrier(DS_READ, 2, 0); 
        }
        
        for (int i = 0; i < cluster_mfma_count_val; i++) {
            schedule_group_barrier(MFMA, 4, 0);
            schedule_group_barrier(DS_READ, 1, 0); 
        } 
    }
