...
void micro_tk(const micro_globals g) {
    ...
    rt_fl<BLOCK_SIZE, BLOCK_SIZE, col_l, rt_32x32_s> C_accum1;
    rt_fl<BLOCK_SIZE, BLOCK_SIZE, col_l, rt_32x32_s> C_accum2;
    ...
    if (is_consumer) {zero(C_accum1); zero(C_accum2);}
    ...
    if (is_consumer) {
        store(g.c, C_accum1, {0, 0, row + consumer_idx, col + local_warp_id});
        store(g.c, C_accum2, {0, 0, row + consumer_idx, col + local_warp_id + 4});
    }
}
