...
void micro_tk(const micro_globals g) {
    ...
    ST_A (&As)[3][M_BLOCK][2] = al.allocate<ST_A, 3, M_BLOCK, 2>();
    ST_B (&Bs)[3][N_BLOCK][2] = al.allocate<ST_B, 3, N_BLOCK, 2>();
    ...
    if (is_producer) {
        #pragma unroll
        for (int m = 0; m < M_BLOCK; m++) {
            G::load<2, false>(As[n1][m][0], g.a, {0, 0, row*2 + 2*m + 0, 1}, swizzled_offsets_A);
            G::load<2, false>(As[n1][m][1], g.a, {0, 0, row*2 + 2*m + 1, 1}, swizzled_offsets_A);
        }
        #pragma unroll
        for (int n = 0; n < N_BLOCK; n++) {
    ...
    for (int tile = 0; tile < num_tiles-2; ++tile) {
        ...
        if (is_producer) {
            #pragma unroll
            for (int m = 0; m < M_BLOCK; m++) {
                G::load<2, false>(As[n2][m][0], g.a, {0, 0, row*2 + 2*m + 0, tile + 2}, swizzled_offsets_A);
                G::load<2, false>(As[n2][m][1], g.a, {0, 0, row*2 + 2*m + 1, tile + 2}, swizzled_offsets_A);
            }
            #pragma unroll
            for (int n = 0; n < N_BLOCK; n++) {
