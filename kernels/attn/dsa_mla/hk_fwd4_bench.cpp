// hk_fwd4_bench.cpp — full-grid benchmark for hk_fwd4 vs gluon (T=4096,H=128,TOPK=1152).
...
int main(int argc,char**argv){
    ...
    size_t shmem=131072;
    HC(hipFuncSetAttribute((void*)hk_fwd4, hipFuncAttributeMaxDynamicSharedMemorySize, shmem));
    dim3 grid(T, HG), block(NT);

    // warmup + timing
    for(int i=0;i<5;i++) hk_fwd4<<<grid,block,shmem>>>(g);
    HC(hipDeviceSynchronize());
    hipEvent_t a,b; hipEventCreate(&a); hipEventCreate(&b);
    const int IT=20;
    hipEventRecord(a);
    for(int i=0;i<IT;i++) hk_fwd4<<<grid,block,shmem>>>(g);
    hipEventRecord(b); HC(hipEventSynchronize(b));
    float ms=0; hipEventElapsedTime(&ms,a,b); ms/=IT;
    printf("HK_FWD4 BENCH: %.4f ms/iter  (grid %dx%d, %d programs)  [gluon ref 3.10 ms]\n",
           ms, T, HG, T*HG);
