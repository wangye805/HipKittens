// hk_s2_occ2_bench.cpp — cyc/tile for the [4x2] occ-2 kernel. Tkg sized [NCTA] (uses blockIdx.x).
...
int main(int argc,char**argv){
    ...
    g_t g{ gl<bf16,-1,-1,-1,-1>(dQ,1,NW_MG,QB,D), gl<bf16,-1,-1,-1,-1>(dKV,1,1,T_KV,D),
           gl<bf16,-1,-1,-1,-1>(dO,1,NW_MG,QB,D), gl<int,-1,-1,-1,-1>(dT,NCTA,1,NTILES,TILE_K),
    ...
    auto launch=[&]{ hk_s2_occ2<<<dim3(NCTA),dim3(NT),shmem>>>(g); };
    ...
    printf("hk_s2_occ2 DC=%d NW=%d(MG=%d): NCTA=%d NTILES=%d TILE_K=%d | %.4f ms/launch, %.1f cyc/tile @ %.0fMHz (occ1 steady 6104; Leon 5396; occ2 model ~3139)\n",
           DC, NW, NW_MG, NCTA, NTILES, TILE_K, ms/IT, cpt, sclk);
    return 0;
}
