// hk_fwd5_bench.cpp — full-grid benchmark for hk_fwd5 vs gluon (T=4096,H=128,TOPK=1152).
...
int main(int argc,char**argv){
    ...
    // warmup + timing
    for(int i=0;i<5;i++) hk_fwd5<<<grid,block,shmem>>>(g);
    HC(hipDeviceSynchronize());
    hipEvent_t a,b; hipEventCreate(&a); hipEventCreate(&b);
    const int IT=20;
    hipEventRecord(a);
    for(int i=0;i<IT;i++) hk_fwd5<<<grid,block,shmem>>>(g);
    hipEventRecord(b); HC(hipEventSynchronize(b));
    float ms=0; hipEventElapsedTime(&ms,a,b); ms/=IT;
    printf("HK_FWD5 BENCH: %.4f ms/iter  (grid %dx%d, %d programs)  [gluon ref 3.10 ms]\n",
           ms, T, HG, T*HG);
