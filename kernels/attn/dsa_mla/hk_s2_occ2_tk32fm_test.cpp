// hk_s2_occ2_tk32fm_test.cpp — correctness for the subtile-streamed kernel. Same bf16-faithful CPU ref.
#include "hk_s2_occ2_tk32fm.cpp"
...
int main(int argc,char**argv){
    ...
    size_t shmem=2*sizeof(KS);   // tight: actual ks[2] at TILE_K=32 = 64KB
    HC(hipFuncSetAttribute((void*)hk_s2_occ2_tk32fm, hipFuncAttributeMaxDynamicSharedMemorySize, shmem));
    hk_s2_occ2_tk32fm<<<dim3(1),dim3(NT),shmem>>>(g); HC(hipDeviceSynchronize());
    ...
    printf("HK_S2_OCC2_TK32SB DC=%d nt=%d sink=%d ninv=%d seed=%d: O worst=%.5f mean=%.6f >.02=%d >.03=%d nan=%d | LSE worst=%.6f  %s\n",
