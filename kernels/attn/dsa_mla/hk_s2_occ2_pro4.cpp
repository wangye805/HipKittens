// hk_s2_occ2_pro4.cpp — ASYMMETRIC barriers: WG0 stops ONLY before PV, WG1 ONLY after PV.
...
__global__ void hk_s2_occ2_pro4(const g_t g){
    ...
    // (E1) hoist sink HBM load to kernel top -> its latency flies during the whole loop,
    //      so the epilogue's skew-balance barrier wait no longer stalls on it.
    typename ST_::row_vec sink_h;
    if (g.has_sink) load(sink_h, g.Sg, coord<>{0,0,warpid,0});
