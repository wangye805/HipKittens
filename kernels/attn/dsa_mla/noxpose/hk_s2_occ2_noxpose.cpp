// hk_s2_occ2_noxpose.cpp — ASYMMETRIC barriers: WG0 stops ONLY before PV, WG1 ONLY after PV.
...
__global__ void hk_s2_occ2_noxpose(const g_t g){
    ...
    // PER-CHANNEL transpose+store: relabel a [DC,QB] col_l channel -> [QB,DC] row_l with a small
    // 16-VGPR temp, vs the full acc_t[16,512] (128 VGPR) that would live alongside acc (128 VGPR)
    // = 256-reg peak at the cap (serialization). transpose here is the free subtile relabel.
    using OcT = rt<float, QB, DC, row_l, rt_16x16_s>;           // [QB=16, DC=64] row_l channel
    #pragma unroll
    for (int c = 0; c < NCH; c++) {
        auto& acc_c = subtile_inplace<DC>(acc, c);              // [DC=64, QB=16] col_l channel
        OcT tt;
        transpose(tt, acc_c);
        store(g.Og, tt, coord<OcT>{0, warpid, 0, c});          // TILE-TYPED coord -> c scaled by cols
    }
