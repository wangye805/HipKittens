...
hipcc -DKITTENS_CDNA4 --offload-arch=gfx950 -std=c++20 -O3 -g -gline-tables-only \
  -I$HK/include -DNTILES=$NTILES -DTILE_K=64 -DTK_SUB=32 -DDC=64 -DNW=8 -DGRID_N=$GRID_N \
  -DHIP_ENABLE_WARP_SYNC_BUILTINS $F hk_s2_occ2b_att.cpp -o ${TAG}_att
...
HIP_VISIBLE_DEVICES=$DEV rocprofv3 --att --att-library-path /opt/rocm/lib --att-target-cu 1 \
  --att-simd-select 0xF --kernel-include-regex hk_s2_occ2b -d ${TAG}_out \
  -- ./${TAG}_att 2>&1 | tail -4
