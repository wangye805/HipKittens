// hk_s2_occ2_stag2ab.cpp — ASYMMETRIC barriers: WG0 stops ONLY before PV, WG1 ONLY after PV.
...
#define GATHER_HALF(buf, ti) gather_kv_async<NT>((buf), g.KVg, topk_all + (ti)*TILE_K, 0, g.kv_rows)
#define SB() __builtin_amdgcn_sched_barrier(0)
#define BAR() do { __builtin_amdgcn_s_barrier(); SB(); } while(0)

__launch_bounds__(NT,MINWAVES)
__global__ void hk_s2_occ2_stag2ab(const g_t g){
    ...
    // ---- prologue: prime tiles 0,1 (both halves) then FULLY drain + sync so both halves are
    //      visible before the skew (steady-state tiles get many barriers of drain slack). ----
    if (wg == 0) GATHER_HALF(ks[0], 0);
    if (wg == 1) GATHER_HALF(ks[0], 0);
    if (nt > 1) { if (wg == 0) GATHER_HALF(ks[1], 1); if (wg == 1) GATHER_HALF(ks[1], 1); }
    __builtin_amdgcn_s_waitcnt(0);
    __syncthreads();

    // ---- SKEW: WG1 does one extra barrier → runs one phase behind WG0 for the whole loop. ----
    if (wg == 1) __builtin_amdgcn_s_barrier();
