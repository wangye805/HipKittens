// hk_s2_occ2_test.cpp — correctness for the [4x2] occ-2 kernel. Same bf16-faithful CPU ref as occ-1.
// Heads per CTA = NW_MG = NW/2 = 4 (each served by 2 sub-warps split over D). BH = QB*NW_MG = 64.
#include "hk_s2_occ2.cpp"
...
int main(int argc,char**argv){
    ...
    const int BH=QB*NW_MG, NKK=NTILES*TILE_K, T_KV=4096;
    ...
    g_t g{ gl<bf16,-1,-1,-1,-1>(dQ,1,NW_MG,QB,D), gl<bf16,-1,-1,-1,-1>(dKV,1,1,T_KV,D),
           gl<bf16,-1,-1,-1,-1>(dO,1,NW_MG,QB,D), gl<int,-1,-1,-1,-1>(dT,1,1,NTILES,TILE_K),
           gl<float,-1,-1,-1,-1>(dS,1,1,NW_MG,QB), gl<float,-1,-1,-1,-1>(dL,1,1,NW_MG,QB),
           NTILES, T_KV, gs, has_sink };
    size_t shmem=131072;
    HC(hipFuncSetAttribute((void*)hk_s2_occ2, hipFuncAttributeMaxDynamicSharedMemorySize, shmem));
    hk_s2_occ2<<<dim3(1),dim3(NT),shmem>>>(g); HC(hipDeviceSynchronize());
    ...
    printf("HK_S2_OCC2 DC=%d nt=%d sink=%d ninv=%d seed=%d: O worst=%.5f mean=%.6f >.02=%d >.03=%d nan=%d | LSE worst=%.6f  %s\n",
