#!/usr/bin/env python3
"""
table_values_16x32.py
Render a 16×32 table where each cell displays a provided value.

Usage:
    python table_values_16x32.py --values values.csv              # saves table_16x32.png
    python table_values_16x32.py --values vals.txt --delimiter ' ' --out my.png

Input format:
  - A text file with rows separated by newlines and columns separated by a delimiter
    (default ','). Must be exactly 16 rows by 32 columns unless overridden via flags.
"""
...
def save_value_table(
    ...
    if swizzle_colors:
        ...
        if swizzle_mode == "by-col":
            ...
            repeat_period = max(2, int(swizzle_period))
...
def parse_args():
    p = argparse.ArgumentParser(description="Render a value-labeled grid image.")
    ...
    p.add_argument("--cols", type=int, default=32, help="Number of columns (default: 32)")
    ...
    p.add_argument("--out", type=str, default="table_16x32.png", help="Output filename (PNG)")
    ...
    p.add_argument("--swizzle-mode", type=str, default="by-col", choices=["by-col", "linear", "row-linear"],
                   help="Pair colors by columns (by-col) or row-major order (linear)")
    p.add_argument("--swizzle-period", type=int, default=128,
                   help="Repeat period in cells for swizzle coloring (default: 128)")
    return p.parse_args()
...
def main():
    ...
    save_value_table(
        ...
        shade=args.shade,
        shade_colors=[s.strip() for s in args.shade_colors.split(",")] if args.shade_colors else None,
        shade_size=args.shade_size,
        darken=[tuple(map(int, p.split(":"))) for p in args.dark.split(";") if p.strip()] if args.dark else None,
        ...
        swizzle_period=args.swizzle_period,
        dpi=300,
    )
