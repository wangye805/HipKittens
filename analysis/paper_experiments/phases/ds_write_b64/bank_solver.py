#!/usr/bin/env python3
"""
Bank Solver for ds_write_b64: Determine the number of LDS banks by testing when addresses wrap around.

Strategy:
- Pick 2 threads from the same phase (so they can conflict)
- Thread 0 writes to banks 0, 1 (offset 0, ds_write_b64 writes 8 bytes = 2 banks)
- Thread 1 writes to progressively higher banks until we detect a conflict
...
def solve_num_banks():
    ...
    print("=" * 70)
    print("Bank Solver (ds_write_b64): Determining the number of LDS banks")
    print("=" * 70)
    print()
    ...
    # Thread 0 always writes to offset 0 (banks 0, 1)
    # ds_write_b64 writes 8 bytes (2 floats), so it accesses 2 consecutive banks
    offset_0 = 0
    banks_0 = [0, 1]  # Bank = (byte_offset / 4) % NUM_BANKS
    
    print(f"Thread {thread_0}: offset={offset_0} bytes → banks {banks_0}")
    print()

    # Test thread 1 at progressively higher bank numbers
    # We start at bank 2 to avoid the initial overlap with banks 0, 1
    # ds_write_b64 writes 2 banks, so thread 1 at bank N covers banks N, N+1
    ...
    test_banks = list(range(2, max_bank_to_test, 1))  # Step by 1 to test every bank
    ...
    for bank in conflict_banks:
        # Check which of thread 1's banks (bank, bank+1) overlap with thread 0's (0, 1)
        wrapped = [(bank + i) % num_banks for i in range(2)]
        overlaps = [b for b in wrapped if b in [0, 1]]
        if overlaps:
            expected_conflicts.append(bank)
            print(f"  Bank {bank}: writes banks {[bank, bank+1]} → wraps to {wrapped} → overlaps at {overlaps} ✓")
    ...
    with open('bank_results.txt', 'w') as f:
        f.write("LDS Bank Detection Results (ds_write_b64)\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Number of LDS banks: {num_banks}\n\n")
        f.write(f"Methodology:\n")
        f.write(f"  - Thread {thread_0} writes to banks {banks_0} (offset {offset_0})\n")
        ...
        for bank in conflict_banks[:10]:  # Show first 10
            wrapped = [(bank + i) % num_banks for i in range(2)]
            overlaps = [b for b in wrapped if b in [0, 1]]
            f.write(f"  Bank {bank}: {[bank, bank+1]} → {wrapped} → overlap {overlaps}\n")
