#!/usr/bin/env python3
"""
Bank Solver: Determine the number of LDS banks by testing when addresses wrap around.

Strategy:
- Pick 2 threads from the same phase (so they can conflict)
- Thread 0 reads from banks 0, 1, 2 (offset 0, ds_read_b96 reads 12 bytes = 3 banks)
...
def solve_num_banks():
    ...
    print("=" * 70)
    print("Bank Solver: Determining the number of LDS banks")
    print("=" * 70)
    print()
    ...
    # Thread 0 always reads from offset 0 (banks 0, 1, 2)
    # ds_read_b96 reads 12 bytes (3 floats), so it accesses 3 consecutive banks
    offset_0 = 0
    banks_0 = [0, 1, 2]  # Bank = (byte_offset / 4) % NUM_BANKS
    
    print(f"Thread {thread_0}: offset={offset_0} bytes → banks {banks_0}")
    print()

    # Test thread 1 at progressively higher bank numbers
    # We start at bank 4 to avoid the initial overlap with banks 0, 1, 2
    # ds_read_b96 reads 3 banks, so thread 1 at bank N covers banks N, N+1, N+2
    ...
    test_banks = list(range(3, max_bank_to_test, 1))  # Step by 1 to test every bank
    ...
    for test_num, bank_1 in enumerate(test_banks, 1):
        ...
        banks_1 = [bank_1, bank_1 + 1, bank_1 + 2]
    ...
    for bank in conflict_banks:
        # Check which of thread 1's banks (bank, bank+1, bank+2) overlap with thread 0's (0, 1, 2)
        wrapped = [(bank + i) % num_banks for i in range(3)]
        overlaps = [b for b in wrapped if b in [0, 1, 2]]
        if overlaps:
            expected_conflicts.append(bank)
            print(f"  Bank {bank}: reads banks {[bank, bank+1, bank+2]} → wraps to {wrapped} → overlaps at {overlaps} ✓")
    ...
    with open('bank_results.txt', 'w') as f:
        f.write("LDS Bank Detection Results\n")
        f.write("=" * 70 + "\n\n")
        ...
        for bank in conflict_banks[:10]:  # Show first 10
            wrapped = [(bank + i) % num_banks for i in range(3)]
            overlaps = [b for b in wrapped if b in [0, 1, 2]]
            f.write(f"  Bank {bank}: {[bank, bank+1, bank+2]} → {wrapped} → overlap {overlaps}\n")
