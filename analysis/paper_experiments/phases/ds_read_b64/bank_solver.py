#!/usr/bin/env python3
"""
Bank Solver for ds_read_b64: Determine the number of LDS banks by testing when addresses wrap around.

Strategy:
- Pick 2 threads from the same phase (so they can conflict)
- Thread 0 reads from banks 0, 1 (offset 0, ds_read_b64 reads 8 bytes = 2 banks)
...
def solve_num_banks():
    ...
    print("=" * 70)
    print("Bank Solver (ds_read_b64): Determining the number of LDS banks")
    print("=" * 70)
    print()
    ...
    # Thread 0 always reads from offset 0 (banks 0, 1)
    # ds_read_b64 reads 8 bytes (2 floats), so it accesses 2 consecutive banks
    offset_0 = 0
    ...
    # ds_read_b64 reads 2 banks, so thread 1 at bank N covers banks N, N+1
    ...
    for bank in conflict_banks:
        ...
        if overlaps:
            ...
            print(f"  Bank {bank}: reads banks {[bank, bank+1]} → wraps to {wrapped} → overlaps at {overlaps} ✓")
    ...
    with open('bank_results.txt', 'w') as f:
        f.write("LDS Bank Detection Results (ds_read_b64)\n")
        f.write("=" * 70 + "\n\n")
